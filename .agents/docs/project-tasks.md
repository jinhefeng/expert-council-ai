...
## IN-PROGRESS
- [x] (2026-06-23) 🔮 弹窗文案优化与主持人流式流畅输出改进：优化新建/编辑会议弹窗中的不规范 Label、占位符引导和按钮，实现通用的流式残损 JSON 属性值提取底座以剥离流式主持人总结期间的裸露大括号，并重构抽象通用结构化卡片渲染组件以复用专家与主持人的 Loading/卡片展示逻辑 (100% 静态类型编译通过，交互极具品质感 ✅)
  - [x] 1. 在 `content-parser.ts` 中重构并导出通用的 `extractStreamingJsonKey(text, key)` 流式 JSON 键提取函数。
  - [x] 2. 在 `ChatMessageCard.tsx` 中导入并在剥离了 `<think>` 标签后对主持人的流式文本进行 summary 提取清洗，完美实现流式正文只展示 Markdown 文本。
  - [x] 3. 对 `page.tsx` 中新建/编辑专家评审圆桌弹窗的标题、各个表单 Label、输入框 Placeholder 占位符、引导语和确认提交按钮文案进行全面对齐优化与美化。
  - [x] 4. 在 `ChatMessageCard.tsx` 中封装通用的结构化数据卡片 `StructuredResultCard`，重构专家立场卡片并实现主持人总结卡片及 Loading 动画的优雅渲染。
  - [x] 5. 全局静态编译验证。
- [x] (2026-06-23) 🔮 机制与交互体验深度优化：发言过程中禁用专家席位与控制胶囊（与原机制对齐），但在追问和决策暂停时解禁以允许调整参数，且实现新专家和新设置的实时数据流捕获，同时在运行中保持历史消息气泡编辑锁定 (100% 静态类型编译通过，交互与流转完美 ✅)
  - [x] 1. 声明 `isSessionPaused`（追问/决策挂起中）和 `isControlsDisabled`（发言及提炼进行中且未暂停）计算属性。
  - [x] 2. 重构大循环内的参会专家列表为动态 `getSelectedExperts()`，确保暂停解禁时增删的专家可在下一步中被实时捕获并正确流转。
  - [x] 3. 升级专家席位的新建专家、卡片勾选、强度 Range 调节、编辑与删除按钮的锁定状态（锁定下禁用且设置 cursor: not-allowed，并增加半透明过渡动画）。
  - [x] 4. 将会议控制胶囊内除提炼结论按钮外的所有配置 select 的禁用属性升级为 `isControlsDisabled`。
  - [x] 5. 消息气泡右上角编辑按钮保持原 `isSessionActive` 锁定状态，防范运行中改写历史。
  - [x] 6. 修复并加固大循环的状态回写机制（更新 `saveCurrentMeetingState` 和 `meetingLoop` 迭代头部以同步包括专家席位在内的所有控制参数），彻底解决暂停修改在流转后发生回退还原的严重缺陷。
  - [x] 7. 防御性加固与优雅中止：引入基于 Web Streams 规范的主动 cancellation 断流设计（在 reader 上监听 abort 并调用 `reader.cancel()`）以从源头消除 `BodyStreamBuffer was aborted` 崩溃隐患；采用声明式 UI 上下文守卫机制（`discussingMeetings &&`）使智能指派、追问研判及方向提炼等子 Loading 在叫停后自动 100% 物理清除；并开发基于 JSON 属性闭合扫描的 `isStreamingJsonKeyClosed`，优化主持人总结 loading 条件使其在正文 `summary` 吐字完毕后才允许展示，从而在交互上与专家保持高度一致。
  - [x] 8. 静态编译验证。
- [x] (2026-06-23) 🔮 功能开发与优化：清洗辅助生成描述中的 think 块 (已物理删除自动匹配专家功能，恢复为原生手动勾选) (100% 静态编译通过，E2E 验证正常 ✅)
  - [x] 1. 在后端 API `/api/discussions/assist` 中封装通用 `<think>` 清洗函数，确保 `meeting_description` 返回不夹杂思维链或未闭合标记。
- [x] (2026-06-23) 🐛 缺陷修复：解决决策方案提炼时后缀杂质和多余括号引起的 JSON.parse 崩溃 SyntaxError 漏洞 (引入平衡括号词法提取与 cleanAndParseJson 双自愈引擎，100% 静态编译通过并验证 ✅)
- [x] (2026-06-23) 🐛 缺陷修复：修复内置专家 think 过程丢失（提示词模板还原与 Loading 判定逻辑优化）、信息澄清面板 markdown 渲染退化，以及专家立场卡片提炼 Loading 出现时机过早的问题。同时打通了 think 在非流式下的原生 reasoning_content 读取并拓宽流式多引擎兼容性；修复了因 think 未闭合及正文未吐字引发立场 loading 提前浮现的 bug；修复了大模型在总结中误将 `]` 错写为 `}` 以及遇到嵌套大括号导致主持人决策卡片提取失败的 Parser 状态机自愈缺陷；实现了问询追问控制台过滤前置英文杂质的末位截取算法 (100% 类型编译通过，交互与解析精度极高 ✅)
- [x] (2026-06-23) 🎨 UI 与交互体验优化：优化专家发言后“立场摘要卡片”提取真空期的 Loading 动画，以及智能体新建/编辑弹窗中“AI自动补全人设”的 Loading 展示与表单锁定防错联锁 (100% 静态类型校验通过，体验无缝丝滑 ✅)
- [x] (2026-06-22) 🐛 缺陷修复：修复气泡重新编辑生成功能因 baseHistory 空白导致整场会议历史上下文丢失的严重 Bug (100% 静态编译通过，精准保留前置历史，清空后半段并重新生成 ✅)
- [x] (2026-06-22) 🐛 缺陷修复与体验优化：修复叫停时澄清面板不消失 Bug，并优化专家提示词段落隔离排版以防上下文混淆与信息冗余
- [x] (2026-06-22) 🐛 缺陷修复：修复非IT会议信息追问偏离主题与补充回答缺乏问答上下文关联的两个缺陷
- [x] (2026-06-22) 🐛 缺陷修复：修复外部与内置智能体流式返回前端气泡无输出、直至全部返回才一次性展示的 React.memo 状态污染 Bug
- [x] (2026-06-22) 🔮 交互与性能深水区重构：整合气泡一体化占位 Loading，物理解决主持人双卡片重合 Bug，并引入 React.memo 与节流滚动大幅削减 Reflow 以消除页面卡顿 (100% 静态编译通过，流式打字丝滑无卡顿 ✅)
- [x] (2026-06-22) 🐛 缺陷修复：修复主持人总结阶段当流以大括号 `{` 开头时导致 `cleanStreamingJson` 与 `extractAndCleanJson` 发生 `lastIndexOf` 死循环卡死 CPU 的漏洞 (已增加 searchPos <= 0 前置拦截，完美复原流畅流式传输 ✅)
- [x] (2026-06-22) 🎨 UI 体验优化：将追问控制台（Inquiry Console）的配色和指示灯统一为主持人总结卡片的主题琥珀色 (面板、指示灯及提交按钮完美统一，视觉极具品质感 ✅)
- [x] (2026-06-22) 🎨 UI 体验优化：统一会议控制胶囊为主题琥珀色，并修复用户消息编辑按钮的 hover 隐藏逻辑 (100% 静态编译通过，外观极其优雅一致 ✅)
  * [x] 1. 统一会议控制胶囊下拉框的配色，将原本分散的红、绿、蓝、黄多色下拉菜单全部收拢为优雅一致的主题琥珀色。
  * [x] 2. 修复用户消息编辑按钮，当会话未结束时物理隐藏不渲染，会话结束后仅在 hover 时渐显，解决之前内联 opacity 覆盖 CSS hover 的缺陷。
- [x] (2026-06-21) 🔮 机制与体验深度优化：前置轮次超限研判、干预文案精细化、主持与追问环连锁联动、追问与决策面板 Loading 交互，以及大文本滚动跟随防断修复 (已全面验证，100% 静态编译通过 ✅)
  * [x] 1. **前置超限研判**：在 `handleSubmitDiscussion` 大循环中，专家发言完毕、主持人流式总结完成后，立即检验 `currentRound`。若超限则直接提示并跳出循环，避免展现选项看板进行无效选择。
  * [x] 2. **精确干预文案**：在决策面板的 Promise 抉择返回载荷中，添加 `isAuto` 属性；倒计时自动执行置 `isAuto === true`，人类手动点击置 `isAuto === false`，并修改消息气泡展示文案。
  * [x] 3. **默认与文案统一**：统一全站为“被动传统”、“协调引导”、“自主决策”三态文案；创建会议默认采用“协调引导”；信息追问默认开启。
  * [x] 4. **连锁联动控制**：在创建会议及会议设置中，当主持模式为“自主决策”时，信息追问强制关闭并禁用；在事件循环中对“自主决策”强制关闭追问逻辑。
  * [x] 5. **双 Loading 友好态组件**：在 `/api/discussions/inquiry` (追问判定中) 和 `/api/discussions/decision-options` (决策选项提炼中) 请求悬挂时，在聊天流底部展示对应的高颜值微磨砂 Loading 卡片。
  * [x] 6. **滚动跟随 Bug 优化**：将自动滚动的触底阈值从原先的狭窄像素拓宽到 150px；同时在流式吐字 delta 及关键布局切换时引入双延迟（double raf/setTimeout）进行滚动纠偏，保证在 KaTeX 及 Markdown 异步排版计算完毕后容器完美跟随触底。

- [x] (2026-06-21) 🔮 机制加固与 Bug 修复 (v2)：决策面板顶端对齐滚动、大循环实时设置同步（追问跳过与中途修改失效修复）、被动模式自动提炼结论叫停 (已全面通过 `npx tsc --noEmit` 静态类型验证，交互非常友好 ✅)
  * [x] 1. **滚动对齐顶端**：在“决策协调看板”生成渲染时，延迟 150ms 触发 `scrollIntoView({ behavior: "smooth", block: "start" })`，将看板顶端平滑对齐视口。
  * [x] 2. **大循环实时设置同步**：引入 `meetingsRef` 并在 `handleSubmitDiscussion` 大循环的各个写入及判定阶段同步拉取外部最新配置，避免过期快照回写覆盖最新主持与追问设置，彻底根除“追问跳过后主持模式自动变更为被动传统”和“中途修改主持模式不生效”的缺陷。
  * [x] 3. **叫停被动/超限模式下自动提炼**：将 `shouldAutoConclude` 初始默认置为 `false`，仅当人类在决策协调看板明确点击“结案陈词”时才置 `true` 触发，把结论提炼与归档的控制权完全归还给用户。


- [x] (2026-06-21) 🎨 体验优化与逻辑绑定：去除多维权衡决策看板、将结案动作与生成结论自动绑定，并恢复原版简洁最终报告 UI (已完成，100% 静态编译通过，结案即自动生成结论并完美渲染原版 Markdown 气泡 ✅)
  * [x] 1. 在 `src/app/page.tsx` 中物理移除「人类主导多维权衡决策看板」的 HTML 渲染与相关状态及“通过/驳回”按钮。
  * [x] 2. 重构 `handleGenerateConclusion` 使其支持接收 `customMeeting?: Meeting`，防御 React 状态滞后。
  * [x] 3. 在 `handleSubmitDiscussion` 大循环 `meetingLoop` 正常跳出后，利用定时器自动触发 `handleGenerateConclusion(currentMeeting)`。
  * [x] 4. 恢复最终结论渲染 UI 至加入追问和决策机制之前的原版极简 Markdown 金色气泡卡片样式（含编辑、保存和解锁）。
...
- [x] (2026-06-21) 🔮 架构重构：流式主持人与 RAG 知识外挂专家及人类决策看板系统 (已验证通过，流式输出完成，三态反馈环和决策看板落地，100% 静态编译通过 ✅)
  * [x] 1. 制定并获得对《功能与交互说明书》和《实施计划》的确认（LGTM）。
  * [x] 2. 实现后端检索专家 RAG 知识外挂（支持配置 dify/向量接口及 5s 超时阻断），并在请求 `/api/discussions/expert-turn` 时作为 RAG 客观事实提示词注入上下文。
  * [x] 3. 重构 `/api/discussions/synthesis` API 端点，支持流式 Event Stream 输出主持人的思考（Reasoning）与正文，并在前端 `page.tsx` 对接流式渲染。
  * [x] 4. 将会议底部的最终结论面板重构为“人类主导多维权衡决策看板”（聚合展示 decisions、tradeoffs 与 concerns），并在底部加入“通过/驳回整改”状态存档以及决议报告生成编辑。
  * [x] 5. 在数据模型中新增 RAG 和 Autonomy 相关字段，并在专家管理弹窗 `ExpertModal.tsx` 中支持可视化 RAG 参数配置。
...
### TODO
- [ ] 📝 建立系统 UX 设计规范，并进行全站 UI/UX 梳理与视觉走查（以实现极致、规范的毛玻璃与主题配色规范设计）
...
- [x] (2026-06-22) 🎨 控制台禁用控制、选项重构与主持风格可见性优化 (v4)
  - [x] 1. 统一封装并复用 `isSessionActive` 常量逻辑，在讨论和提炼进行时全局禁用胶囊面板 select 下拉、提炼按钮以及用户消息右上角“重新编辑”编辑按钮，叫停时自动解除。
  - [x] 2. 调整胶囊控制台下拉选项横向排布顺序为“风格 ➔ 机制 ➔ 模式”，更名并统一流转发言机制文案为“顺序发言”、“动态指派”和“点名发言”，同步对齐新建弹窗、后台管理及帮助信息。
  - [x] 3. 优化主持风格在聊天气泡的可见性，生成主持人或 AI 自主选择气泡时，动态以选中的风格名字（如“严厉评审型”）作为 senderName，使配置在前端直观可见。
  - [x] 4. 同步更新了技术规范《功能与 RAG 专家说明书》及用户《使用说明手册 (manual.html)》，增加 2.6 新版控制台说明。
