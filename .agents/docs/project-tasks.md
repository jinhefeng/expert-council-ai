...
### IN-PROGRESS
- [ ] (2026-06-21) 🎨 体验优化：后台管理页面 UI/UX 体验重构与垂直页签式（Sidebar Tabs）极简信息架构升级
  * [ ] 1. 在 `admin/page.tsx` 主体区重构为左侧垂直页签导航、右侧条件内容展示的两栏式信息布局，整合身份预设与全局默认值。
  * [ ] 2. 在提示词模板管理页签中设计“子页签 (Sub-tabs)”分区展示，将 14 个大提示词框按会议流阶段进行分组展示以减少视觉压迫。
  * [ ] 3. 优化卡片 Hover 微平移与阴影动效，并为所有文本框及 textarea 添加聚焦（:focus）发光缓动动画，保持毛玻璃美学风格一致。
