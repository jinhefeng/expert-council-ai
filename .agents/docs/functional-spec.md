...
3. **附带卡片结构化与清洗引擎**：在流结束后，前端与后端对文本内容进行深度清洗与结构化提取：
   - **多 JSON 循环清洗机制**：支持循环查找并剔除文本中残存的所有合法/未闭合 JSON 块，确保在分段式流累积时，所有残留的大括号均被精准摘除。
   - **代码块标记自适应吞并**：在提取 JSON 文本时，向上合并前置的代码块开始符（如 \`\`\`json），向下合并后置的闭合符（如 \`\`\`），并作为一个整体从原文中完全抹除，彻底解决由于未闭合代码块导致 Markdown 渲染时底部残留大片灰色空白空框的问题。
   - **连续汉字前导英文独白清除**：系统在匹配英文思考指令泄漏时，通过定位首个长度不小于 5 的连续汉字块来预测真正中文发言的起点，规避因思考中带有专家名字（如“小蔚”、“董事长”）所导致的过早切断与大量英文泄漏问题。
