...
class DesignCouncilAdapter {
  ...
  connect() {
    ...
    console.log(`[DesignCouncil-Channel] Connecting to platform at ${url}...`);
    this.ws = new WebSocket(url);

    this.ws.on("open", () => {
      console.log("[DesignCouncil-Channel] Successfully connected to Design Council Platform!");
    });
    ...
    this.ws.on("message", async (data) => {
      try {
        ...
        console.error("[DesignCouncil-Channel] Error handling WebSocket message:", e);
      }
    });

    this.ws.on("close", () => {
      console.log("[DesignCouncil-Channel] Connection lost, reconnecting in 5s...");
      setTimeout(() => this.connect(), 5000);
    });

    this.ws.on("error", (err) => {
      console.error("[DesignCouncil-Channel] WebSocket error:", err.message);
    });
  }

  async handleTurnActive(data) {
    // data: { meetingId, turnId, question, context, previousTurns }
    console.log(`[DesignCouncil-Channel] Turn activated for meeting: ${data.meetingId}, turn: ${data.turnId}`);
    
    // Create or find an OpenClaw Session for this meeting
    const sessionKey = `dc-${data.meetingId}`;
    this.sessionMapping.set(sessionKey, data.turnId);

    // Normalize the input message from Design Council into OpenClaw's internal Message format
  ...
  send(sessionKey, message) {
    ...
    if (!turnId) {
      console.warn(`[DesignCouncil-Channel] No turnId mapping found for session: ${sessionKey}`);
      return;
    }
    ...
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      if (message.isStream) {
        ...
        console.log(`[DesignCouncil-Channel] Completed turn: ${turnId}`);
      }
    } else {
      console.error("[DesignCouncil-Channel] WebSocket is not open. Failed to send message back.");
    }
  }
}

module.exports = DesignCouncilAdapter;
